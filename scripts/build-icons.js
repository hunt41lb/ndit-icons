 ...
 *
 * Directory structure:
 *   icons/brands/streaming/disney-plus.svg  → ndit:brand-streaming-disney-plus
 *   icons/brands/streaming/netflix.svg      → ndit:brand-streaming-netflix
 *
 * Naming convention:
 ...
 * icons/brands/streaming/disney-plus.svg → brand-streaming-disney-plus
 */
function filePathToIconName(filePath) {
  ...
  // Singularize first directory (brands → brand, devices → device)
...
console.log("🎨 NDIT Icons - Building icon map...\n");
...
for (const filePath of svgFiles) {
  ...
  if (!pathData) {
    errors.push(`  ✗ ${iconName}: no <path> elements found`);
    continue;
  }
  ...
  console.log(`  ✓ ${iconName}${viewBox ? ` (${viewBox})` : ""}`);
}
