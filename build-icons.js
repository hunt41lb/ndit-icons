 ...
 * this builder preserves the full SVG inner content — gradients, multiple
 * paths, classes — so multi-color icons render correctly.
 *
 * Usage:
 *   node build-icons.js
 *
...
function parseSvg(filePath) {
  ...
  if (!viewBoxMatch) {
    console.warn(`  ⚠  Skipping ${filePath} — no viewBox found`);
    return null;
  }
  ...
  if (!innerMatch) {
    console.warn(`  ⚠  Skipping ${filePath} — could not parse SVG content`);
    return null;
  }
...
// ---------------------------------------------------------------------------
function main() {
  console.log("🔨 NDIT Icons Builder\n");
  ...
  console.log(`📂 Found ${svgFiles.length} SVG file(s) in ${SVG_SOURCE_DIR}\n`);

  if (svgFiles.length === 0) {
    console.error("❌ No SVG files found. Add icons to icon-svg/ and try again.");
    process.exit(1);
  }
  ...
  if (icons.length === 0) {
    console.error("❌ No valid icons parsed. Check your SVG files.");
    process.exit(1);
  }
  ...
  console.log(`📁 Generated file: ${path.relative(__dirname, OUTPUT_FILE)}`);
  console.log(`🔢 Total icons: ${icons.length}`);
  console.log(`🏷️  Prefix: ${ICON_PREFIX}:`);
}
